##############################################################################################
#' @title Comparison of salt correction for MART v1.0.15 and v1.0.16 of the reaRate package

#' @author
#' Kaelin M. Cawley \email{kcawley@battelleecology.org} \cr

#' @description This script retrieves, joins, and compares salt corrections for two versions 
#' of the reaRate package.
...
#   Kaelin M. Cawley (2024-03-21)
#     original creation
#########################################################################################

#User Inputs
siteID <- "MART"
workingPath <- "C:/Users/kcawley/Desktop/MART_v1_0_16"
plotPathLossRates <- paste0(workingPath,"/saltAndGasLossPlots")
plotPathBackground <- paste0(workingPath,"/backgroundPlots")
plotPathTravelTime <- paste0(workingPath,"/travelTimePlots")
...
qInputList <- neonUtilities::loadByProduct(dpID = dscDPID, 
                                           site = siteID, 
                                           include.provisional = TRUE, 
                                           check.size = FALSE)
...
save.image(file = paste0(workingPath,"/data_", Sys.Date(),".RData"))
workingPath <- "C:/Users/kcawley/Desktop/MART_v1_0_16"
load(paste0(workingPath,"/data_2024-03-21.RData"))

# Format all of the tables into one
# rea_backgroundFieldCondData = reaInputList$rea_backgroundFieldCondData
...
reaFormatted$oldPlateauSaltConcCleanCorr <- NA
reaFormatted$oldMeanPlatSaltConcCleanCorr <- NA
reaFormatted$oldSdPlatSaltConcCleanCorr <- NA
for(i in seq(along = reaFormatted$siteID)){
  saltForBackCor <- as.numeric(unlist(strsplit(reaFormatted$plateauSaltConc[i],split = '\\|')))
  try(reaFormatted$oldPlateauSaltConcCleanCorr[i] <- paste((saltForBackCor / reaFormatted$backgroundSaltConc[i]), collapse = "|"))
  try(reaFormatted$oldMeanPlatSaltConcCleanCorr[i] <- mean((saltForBackCor / reaFormatted$backgroundSaltConc[i]), na.rm = TRUE))
  try(reaFormatted$oldSdPlatSaltConcCleanCorr[i] <- stats::sd((saltForBackCor / reaFormatted$backgroundSaltConc[i]), na.rm = TRUE))
...
if(!dir.exists(plotPathLossRates)){
  dir.create(plotPathLossRates)
}
...
plotPathLossRatesOld <- "C:/Users/kcawley/Desktop/MART_v1_0_16/saltAndGasLossPlotsOld"
plotsOut <- reaRate::gas.loss.rate.plot(inputFile = reaFormatted,
                                        injectionTypeName = "injectionType",
                                        eventID = "eventID",
                                        stationToInjectionDistance = "stationToInjectionDistance",
                                        plateauGasConc = "plateauGasConc",
                                        corrPlatSaltConc = "corrPlatSaltConc",
                                        savePlotPath = plotPathLossRatesOld)
