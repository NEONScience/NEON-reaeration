...
transformed parameters{
  ...
  for (j in 1: nexpt){  // shitty coding on my part to make a loop here
  Ksf6[j] = exp(logk[j]) /  ((600/(3255.3-(217.13*temp[j])+(6.837*temp[j]^2)-(0.08607*temp[j]^3)))^-0.5);
  }
  
  Kd= Ksf6 ./ V;  //check this velocity conversion. m/day
...
model {
  ...
  for (j in 1:nexpt){
    logk[j]~normal( a + b*log(Q[j]) , sigma_expt);
  }
